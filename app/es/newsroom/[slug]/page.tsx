...
export function generateStaticParams() {
  const { pressReleaseDetails } = getContent('es')
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!release) {
    return { title: 'No encontrado | SINOMINING' }
  }

  return {
    title: `${release.title} | Sala de prensa | SINOMINING`,
...
export default async function PressReleasePage({ params }: Props) {
  const { slug } = await params
  const { breadcrumb, pressReleaseDetails } = getContent('es')
  const release = pressReleaseDetails.find((r) => r.slug === slug)
      ...
      <main className="flex-1">
        <BreadcrumbNav
          ...
          items={[
            ...
            { label: breadcrumb.newsroom, href: '/es/newsroom' },
            { label: release.title },
          ]}
        />
