...
export function generateStaticParams() {
  const { productsPage } = getContent('es')
...
export default async function ProductDetailPage({
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <SiteHeader lang="es" altHref={`/products/${slug}`} />
      <main className="flex-1">
        <BreadcrumbNav
          lang="es"
          items={[
            { label: breadcrumb.home, href: '/es' },
            { label: breadcrumb.allProducts, href: '/es/products' },
            { label: product.name },
          ]}
        />

        <ProductDetail lang="es" product={product} />
