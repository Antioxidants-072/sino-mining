...
export function generateStaticParams() {
  const { productsPage } = getContent('ru')
...
export default async function ProductDetailPage({
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <SiteHeader lang="ru" altHref={`/ru/products/${slug}`} />
      <main className="flex-1">
        <BreadcrumbNav
          lang="ru"
          items={[
            { label: breadcrumb.home, href: '/ru' },
            { label: breadcrumb.allProducts, href: '/ru/products' },
            { label: product.name },
          ]}
        />

        <ProductDetail lang="ru" product={product} />
