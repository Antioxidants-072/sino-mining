...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!release) {
    return { title: 'Не найдено | SINOMINING' }
  }

  return {
    title: `${release.title} | Пресс-центр | SINOMINING`,
    description: release.sections[0]?.body?.slice(0, 160),
  }
}

export default async function PressReleaseRu({ params }: Props) {
  const { slug } = await params
  const { breadcrumb, pressReleaseDetails } = getContent('ru')
  const release = pressReleaseDetails.find((r) => r.slug === slug)
      ...
      <main className="flex-1">
        <BreadcrumbNav
          ...
          items={[
            ...
            { label: breadcrumb.newsroom, href: '/ru/newsroom' },
            { label: release.title },
          ]}
        />
